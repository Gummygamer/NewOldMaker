...
fn actors_tab(ui: &mut egui::Ui, project: &mut ProjectData) {
    let skills: Vec<(u32, String)> = project
        .skills
        .iter()
        ...
        egui::CollapsingHeader::new(format!("{} · {} {}", s.id, s.element.icon(), s.name))
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label("Power ×");
                    ui.add(
                        egui::DragValue::new(&mut s.power)
                            .speed(0.05)
                            .range(0.0..=8.0),
                    );
                    ui.label("MP");
                ...
                egui::ComboBox::from_label("Effect")
                    ...
                    .show_ui(ui, |ui| {
                        for (j, name) in ["Damage", "Heal", "Buff attack", "Break defense"]
                            .iter()
                            .enumerate()
                        {
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    for el in ALL_ELEMENTS {
                        ...
                        if ui
                            .toggle_value(&mut on, format!("{} {}", el.icon(), el.name()))
                            .changed()
                        {
                            if on {
...
fn troops_tab(ui: &mut egui::Ui, project: &mut ProjectData) {
    let enemies: Vec<(u32, String)> = project
        .enemies
        .iter()
...
fn system_tab(ui: &mut egui::Ui, project: &mut ProjectData) {
    ...
    egui::ComboBox::from_label("Start map")
        ...
        .show_ui(ui, |ui| {
            let maps: Vec<(u32, String)> = project
                .maps
                .iter()
    ...
    let actors: Vec<(u32, String)> = project
        .actors
        .iter()
...
fn llm_tab(ui: &mut egui::Ui, project: &mut ProjectData) {
    ui.label(RichText::new("LLM for NPC dialogue").strong());
    ui.horizontal(|ui| {
        ui.label("Backend");
        egui::ComboBox::from_id_salt("llm_backend")
            .selected_text(match project.llm.backend {
                LlmBackend::Local => "Local (llama.cpp, offline)",
                LlmBackend::Nim => "NVIDIA NIM (cloud API)",
            })
            .show_ui(ui, |ui| {
                ui.selectable_value(
                    &mut project.llm.backend,
                    LlmBackend::Local,
    ...
    match project.llm.backend {
        LlmBackend::Local => {
            ...
            if ui
                .add(egui::Slider::new(&mut ctx, 512..=8192).text("Context tokens"))
                .changed()
            {
            ...
            if ui
                .add(egui::Slider::new(&mut threads, 0..=32).text("CPU threads (0 = auto)"))
                .changed()
            {
            ...
            ui.colored_label(
                egui::Color32::YELLOW,
                "Engine built without the `llm` feature — fallback lines will be used.",
            );
        }
        LlmBackend::Nim => {
            ui.small("Uses NVIDIA's hosted NIM endpoints (OpenAI-compatible). Requests go to the cloud and need an NVIDIA API key. NPCs with 'LLM dialogue enabled' speak through the chosen model.");
            ui.horizontal(|ui| {
                ui.label("API key");
                ui.add(
                    egui::TextEdit::singleline(&mut project.llm.nim_api_key)
                        .password(true)
                        .hint_text("nvapi-…"),
                );
            });
            ui.small("Leave blank to read the NVIDIA_API_KEY environment variable instead.");
            ui.horizontal(|ui| {
                ui.label("Model");
                ui.text_edit_singleline(&mut project.llm.nim_model);
            });
            ui.small("e.g. meta/llama-3.1-8b-instruct, mistralai/mistral-7b-instruct-v0.3");
            ui.horizontal(|ui| {
                ui.label("Base URL");
                ui.text_edit_singleline(&mut project.llm.nim_base_url);
            });
            #[cfg(not(feature = "nim"))]
            ui.colored_label(
                egui::Color32::YELLOW,
                "Engine built without the `nim` feature — fallback lines will be used.",
            );
        }
    }
    ...
    if ui
        .add(egui::Slider::new(&mut max, 24..=256).text("Max reply tokens"))
        .changed()
    {
